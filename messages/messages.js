...
function updateTabUI() {
  tabCommunity.classList.toggle("active", currentTab === "community");
  tabMessages.classList.toggle("active", currentTab === "messages");
  tabNotifications.classList.toggle("active", currentTab === "notifications");

  // Input visibility
  const showInput = (currentTab === "community") || (currentTab === "messages" && currentConversationId);
  chatInputWrap.style.display = showInput ? "flex" : "none";
}
...
function showToast(title, body) {
  ...
  toast.className = "toast-msg"; // Ensure you add CSS for this if missing, using inline for safety
  toast.style.cssText = `
  ...
  `;
  toast.innerHTML = `<span>ðŸ””</span> <div>${title}</div>`; // Body might be too long
...
async function loadCommunity() {
  chatInput.placeholder = "Post to communityâ€¦";
  ...
  if (error) {
    list.innerHTML = `<div class="item error">Failed to load chat.</div>`;
    return;
  }
  ...
  communitySub = supabase.channel('community_chat_realtime')
    .on('postgres_changes', { event: 'INSERT', table: 'community_chat' }, async (payload) => {
      ...
      if (newMsg && currentTab === "community") {
        ...
        if (!exists) list.prepend(createCommunityMsgEl(newMsg));
      }
    })
    .subscribe();
}
...
async function loadConversations() {
  ...
  // Enhancement: Fetch last message + other participant for EACH conversation
  // Note: This matches the previous logic's N+1 but adds message fetching to improve UI
  const items = [];
...
async function loadPrivateChat(convId) {
  ...
  if (!currentOtherId) {
    ...
    if (other) {
      ...
      chatInput.placeholder = `Message ${other.full_name}...`;
    }
  }
    ...
    div.innerHTML = `
      <div style="display:flex; gap:12px; align-items:flex-start;">
        <div style="padding:10px; border-radius:12px; background:var(--bg-body); box-shadow:var(--shadow-sm); font-size:20px;">
          ${n.type === 'message' ? 'ðŸ’¬' : (n.type === 'message_request' ? 'ðŸ‘‹' : 'ðŸ””')}
        </div>
        <div style="flex:1;">
    ...
    if (error) {
      ...
      if (currentOtherId) {
        sendNotification(
          currentOtherId,
          "New Message",
          `${currentUser.user_metadata?.full_name || currentUser.full_name || 'Someone'} sent you a message.`,

...
function renderPost(row) {
  ...
  const actions = `
    <div class="postFooter">
      <button class="miniBtn ${liked ? "liked" : ""}" type="button" data-action="like" data-post-id="${escapeHtml(
        String(postId)
      )}">
      ...
      <button class="miniBtn" type="button" data-action="share" data-post-id="${escapeHtml(
        String(postId)
      )}">Share</button>
      ${deleteBtn}
    </div>
      ...
      <div class="postHeader">
        <div class="postAuthor" style="display:flex;align-items:center;gap:10px;">
          <a href="${authorLink}" style="display:inline-flex;align-items:center;gap:10px;text-decoration:none;color:inherit;">
            ...
            <div>
              <div class="postAuthorName" style="font-weight:900;line-height:1.1;">
                ${authorName}${isMine ? ` <span class="youTag">you</span>` : ``}
              </div>
...
async function toggleLike(postId) {
  ...
  if (likedErr) {
    alert(`Like failed: ${likedErr.message}`);
    return;
  }
...
feedListEl?.addEventListener("click", async (e) => {
  ...
  if (action === "delete") return await deletePost(postId);
  if (action === "like") return await toggleLike(postId);
  if (action === "comment") return await addComment(postId);
